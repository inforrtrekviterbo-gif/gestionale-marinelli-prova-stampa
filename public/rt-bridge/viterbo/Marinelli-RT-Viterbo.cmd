@echo off
rem ===========================================================================
rem  MARINELLI STEFANO - PONTE REGISTRATORE RT - VITERBO
rem
rem  File unico eseguibile. Doppio clic = installa.
rem  Il ponte intero sta sotto la riga marcatore, dentro questo stesso file.
rem
rem    doppio clic          installa (chiede l amministratore)
rem    ... -Disinstalla     rimuove l avvio automatico
rem    ... -SelfTest        autotest, non serve l amministratore
rem ===========================================================================
setlocal EnableExtensions

set "ARGOMENTI=%*"
if "%ARGOMENTI%"=="" set "ARGOMENTI=-Installa"

rem L amministratore serve solo per installare o disinstallare.
rem L autotest e il ponte stesso girano senza.
set "SERVEADMIN="
echo %ARGOMENTI% | findstr /i /c:"-Installa" >nul && set "SERVEADMIN=1"
echo %ARGOMENTI% | findstr /i /c:"-Disinstalla" >nul && set "SERVEADMIN=1"
if defined SERVEADMIN (
  net session >nul 2>&1
  if errorlevel 1 (
    echo Servono i diritti di amministratore: confermare la richiesta di Windows.
    powershell -NoProfile -Command "Start-Process -FilePath '%~f0' -ArgumentList '%ARGOMENTI%' -Verb RunAs"
    exit /b
  )
)

set "PONTE=%TEMP%\Marinelli-RT-Viterbo.ps1"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$t=Get-Content -Raw -LiteralPath '%~f0'; $m='#--'+'PONTE--#'; $i=$t.IndexOf($m); if($i -lt 0){exit 9}; [IO.File]::WriteAllText('%PONTE%', $t.Substring($i+$m.Length+2), [Text.Encoding]::ASCII)"
if errorlevel 1 (
  echo ERRORE: non riesco a estrarre il ponte da questo file.
  pause
  exit /b 1
)

powershell -NoProfile -ExecutionPolicy Bypass -File "%PONTE%" %ARGOMENTI%
exit /b %errorlevel%

#--PONTE--#
# =============================================================================
# MARINELLI STEFANO - PONTE REGISTRATORE RT - VITERBO
#
# File unico. Contiene il ponte, l'installazione e la disinstallazione.
# Non serve estrarre nessuno ZIP e non serve nessun altro file accanto.
#
#   Installazione   tasto destro sul file > Esegui con PowerShell (da
#                   amministratore), oppure:
#                     powershell -ExecutionPolicy Bypass -File .\Marinelli-RT-Viterbo.ps1 -Installa
#
#   Disinstallazione  ... -Disinstalla
#   Autotest          ... -SelfTest
#
...
param(
  ...
  [string]$Chiave,
  [string]$ConfigPath = (Join-Path $env:ProgramData "MarinelliRTBridge\Viterbo\config.json")
)
...
$script:ConfigurazioneDiPartenza = [ordered]@{
  ...
  RchHost                          = "192.168.1.210"
  RchPort                          = 23
  RchProtocol                      = "Standard"
  RchAddress                       = "01"
  RchDepartment                    = 1
  RchFidelityEnabled               = $true
  RchPaymentIndexes                = [ordered]@{ cash = 1; card = 4; bank = 8; gift = 10 }
  RchConnectTimeoutMilliseconds    = 5000
  RchResponseTimeoutSeconds        = 20
  RchFiscalProtocolValidated       = $true
  FiscalJobsNotBeforeUtc           = ""
  LogPath                          = ""
}

function Stop-ConMessaggio([string]$Messaggio) {
  Write-Host "`nERRORE: $Messaggio" -ForegroundColor Red
...
function Test-Amministratore {
  ...
  return $ruolo.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
}
...
function Set-RiservaPortaLocale([int]$Porta) {
  $prefisso = "http://localhost:$Porta/"
  & netsh.exe http show urlacl "url=$prefisso" *> $null
  if ($LASTEXITCODE -eq 0) {
    & netsh.exe http delete urlacl "url=$prefisso" *> $null
  }
  $sistema = Get-NomeSistema
  & netsh.exe http add urlacl "url=$prefisso" "user=$sistema" | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Impossibile riservare il collegamento locale $prefisso per $sistema." }
  Write-Host "Collegamento locale $prefisso riservato a $sistema." -ForegroundColor Green
}

function Install-PonteViterbo {
  if (-not (Test-Amministratore)) {
    Stop-ConMessaggio "Avvia questo file come amministratore: tasto destro > Esegui con PowerShell, oppure apri PowerShell come amministratore."
  }

  Write-Host "MARINELLI STEFANO - COLLEGAMENTO REGISTRATORE RT" -ForegroundColor Green
  Write-Host "Negozio: $script:Negozio"
  Write-Host "File unico, avvio all'accensione del computer.`n"

  # 1. Autotest del protocollo, prima di toccare qualsiasi cosa.
  Write-Host "Autotest del Protocollo Standard RCH..." -ForegroundColor Cyan
  $script:Config = [pscustomobject]@{ RchAddress = "01" }
  [byte[]]$pacchettoProva = New-RchPacket -Data "=T1" -PacketId ([char]'0')
  if ([BitConverter]::ToString($pacchettoProva) -ne "02-30-31-30-30-33-4E-3D-54-31-30-31-36-03") {
    Stop-ConMessaggio "Autotest creazione pacchetto RCH non superato."
  }
  $script:Config = $null
  ...
  if ([Environment]::UserInteractive) {
    $indirizzo = Read-Host "Indirizzo IP RCH [$($configurazione.RchHost)]"
    if (-not [string]::IsNullOrWhiteSpace($indirizzo)) { $configurazione.RchHost = $indirizzo.Trim() }
    $porta = Read-Host "Porta RCH [$($configurazione.RchPort)]"
    if (-not [string]::IsNullOrWhiteSpace($porta)) { $configurazione.RchPort = [int]$porta }
  }
  Write-Host "Verifica TCP verso $($configurazione.RchHost):$($configurazione.RchPort)..." -ForegroundColor Cyan
  try { $raggiungibile = Test-TcpEndpoint -HostName ([string]$configurazione.RchHost) -Port ([int]$configurazione.RchPort) }
  catch { Stop-ConMessaggio "La cassa RCH non risponde su $($configurazione.RchHost):$($configurazione.RchPort). $($_.Exception.Message)" }
  if (-not $raggiungibile) { Stop-ConMessaggio "La cassa RCH non risponde su $($configurazione.RchHost):$($configurazione.RchPort)." }
  Write-Host "RCH raggiungibile. Nessun comando inviato alla cassa." -ForegroundColor Green

  # 5. Ferma un'installazione precedente, poi copia questo stesso file.
  $attivitaEsistente = Get-ScheduledTask -TaskName $script:NomeAttivita -ErrorAction SilentlyContinue
  if ($attivitaEsistente) {
    Stop-ScheduledTask -TaskName $script:NomeAttivita -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 2
  }
  New-Item -ItemType Directory -Force -Path $script:CartellaInstallazione | Out-Null
  ...
  Set-RiservaPortaLocale -Porta $portaLocale

  # 8. Attivita' pianificata all'accensione, come SISTEMA.
  ...
  $innesco.Delay = "PT1M"
  $impostazioni = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -RestartCount 60 -RestartInterval (New-TimeSpan -Minutes 1) -ExecutionTimeLimit ([TimeSpan]::Zero) -MultipleInstances IgnoreNew
  $utenza = New-ScheduledTaskPrincipal -UserId "S-1-5-18" -LogonType ServiceAccount -RunLevel Highest
  Register-ScheduledTask -TaskName $script:NomeAttivita -Action $azione -Trigger $innesco -Settings $impostazioni -Principal $utenza -Description "Collegamento automatico tra il gestionale Marinelli Stefano e il registratore RT Viterbo. Parte all'accensione del computer." -Force | Out-Null

  Start-ScheduledTask -TaskName $script:NomeAttivita
  Start-Sleep -Seconds 5
  $attivita = Get-ScheduledTask -TaskName $script:NomeAttivita
  $informazioni = Get-ScheduledTaskInfo -TaskName $script:NomeAttivita
  if ($attivita.State -ne "Running") {
    Stop-ConMessaggio "L'avvio automatico non e' riuscito. Codice Windows: $($informazioni.LastTaskResult). Registro: $($configurazione.LogPath)"
  }

  Write-Host "`nINSTALLAZIONE COMPLETATA" -ForegroundColor Green
  Write-Host "Il ponte parte da solo all'accensione del computer, come SISTEMA."
...
function Uninstall-PonteViterbo {
  if (-not (Test-Amministratore)) {
    Stop-ConMessaggio "Avvia questo file come amministratore."
  }
  if (Get-ScheduledTask -TaskName $script:NomeAttivita -ErrorAction SilentlyContinue) {
    Stop-ScheduledTask -TaskName $script:NomeAttivita -ErrorAction SilentlyContinue
    Unregister-ScheduledTask -TaskName $script:NomeAttivita -Confirm:$false
    Write-Host "Avvio automatico rimosso." -ForegroundColor Green
  } else {
    Write-Host "Nessun avvio automatico da rimuovere."
  }
  $prefisso = "http://localhost:8080/"
  & netsh.exe http show urlacl "url=$prefisso" *> $null
  if ($LASTEXITCODE -eq 0) {
    & netsh.exe http delete urlacl "url=$prefisso" *> $null
    Write-Host "Riserva del collegamento locale rimossa."
  }
  if ($EliminaConfigurazione -and (Test-Path -LiteralPath $script:CartellaInstallazione)) {
    Remove-Item -LiteralPath $script:CartellaInstallazione -Recurse -Force
    Write-Host "Configurazione, chiave e registro eliminati." -ForegroundColor Yellow
  } else {
    Write-Host "Configurazione e registro lasciati in $script:CartellaInstallazione (usa -EliminaConfigurazione per toglierli)."
  }
  if ([Environment]::UserInteractive) { Read-Host "Premi INVIO per chiudere" }
  exit 0
}
