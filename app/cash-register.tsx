...
function buildRchReceiptCommands(payload: LocalFiscalPayload | null | undefined) {
  ...
  commands.push("=S");
  commands.push(...buildRchPaymentCommands(payload));
  ...
  return (
    <label className={`field ${compact ? "field-compact" : ""} ${className}`}>
      ...
      <span className="input-wrap">
        ...
        {String(value).length > 0 && <button type="button" className="clear-input" aria-label={`Cancella ${label ?? "campo"}`} onClick={() => onChange("")}><MaterialIcon>close</MaterialIcon></button>}
      </span>
    </label>
  );
}
...
function MaterialIcon({ children }: { children: ReactNode }) {
...
function DepositForm({ products, store, repair, add, close }: { products: Product[]; store: Store; repair: boolean; add: (item: CartItem) => void; close: () => void }) {
  ...
  return <form className="stack" onSubmit={submit}>
    ...
    {error && <div className="alert danger"><MaterialIcon>warning</MaterialIcon>{error}</div>}<p className="muted">Verrà generato un unico PDF con tutti i prodotti e un solo EAN per richiamare rapidamente l’operazione al saldo.</p><div className="form-actions"><button type="button" className="secondary" onClick={close}>Annulla</button><button className="primary">Aggiungi al carrello</button></div>
  </form>;
}
